...
**Status: PARTIAL** — Only `install_decktape.sh` and `decktape_wrapper.sh` set `set -euo pipefail`; **10 of 13 installers still have none**. Deliberately deferred: the audit's own note says strict mode will surface latent failures needing a full two-arch build to diagnose, and no session so far has had Docker.
...
No remediation session so far has had Docker. **`make build` and `make test` have not been run on either architecture** across PRs #104, #105 and #118. Specifically unverified: the DeckTape amd64 path (3.8 — `DECKTAPE_AMD64_HANDOFF.md` in the working tree is the briefing for that), the new OCI labels (4.7), and `--build-arg BUILDARCH` under the legacy builder (4.3).
