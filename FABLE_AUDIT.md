...
**Status: PARTIAL** — Only `install_decktape.sh` and `decktape_wrapper.sh` set `set -euo pipefail`; **10 of 13 installers still have none**, and the shared `_lib.sh` helper does not exist. Deliberately deferred: the audit's own note says strict mode will surface latent failures needing a full two-arch build to diagnose, and no session so far has had Docker. Partial progress from 4.2's lint triage: `install_tippecanoe.sh` gained a shebang, and the `export VAR=$(…)` return-value masking in `install_{quarto,typst,gpq}.sh` is fixed — which is precisely what lets `set -e` catch those failures once it lands. There is now also a **regression guard**: shellcheck runs on every PR at warning severity, so a new `et -e` cannot reach master unnoticed.
