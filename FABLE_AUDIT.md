...
**Date:** 2026-07-07 · **Scope:** full repository at commit `1c5282a`
**Updated:** 2026-07-08 — each finding carries a **Proposed fix** and a **Model** recommendation.
**Updated:** 2026-08-20 — **this is now a living tracker.** Every finding carries a **Status** line; the scoreboard below is the index. Diagnoses are left as originally written (they are the spec for the fix and the historical record); where the repo has since moved, the Status line says so and the repo wins.
...
| ✅* | Done, but a stated validation gate was never met — read the Status line |
| 🟡 | Partially done — the Status line says what is left |
| ⬜ | Open |
| ⛔ | Closed without action (dropped, or won't-fix by policy) |
...
- **DeckTape must work out of the box on both arches.** That guarantee outranks simplification — see 3.8.
- **Validation honesty.** Sessions here routinely lack Docker, hadolint and shellcheck. State what was actually run; never claim a build or test that did not happen.
...
| 6 | `gds_amd64.yml` and `gds_arm64.yml` are a **214-line near-duplicate pair** maintained by hand-commenting; they have already drifted (pins, missing packages) | `env/gds_*.yml` | Maintenance burden + real drift | ⬜ 3.1 |
| 7 | `Makefile` `docker run` lacks `--rm` — every `make test` leaves three exited containers (each holding a copy of the image's writable layer) | `Makefile:3` | Host disk creep | ✅ 4.3 |
| 8 | Version/naming drift: `.gitpod.yml` pins `darribas/gds_dev:7.0` (four majors old, dead image name); `env/*/README.md` still describe the retired `gds_py`/`gds_dev` flavour architecture | `.gitpod.yml`, `env/{py,r,dev}/README.md` | User-facing confusion | ✅ 2.2/2.11 |
| 9 | `install_r_extra.sh` begins with `et -e` (typo for `set -e`); the script is disabled in the Dockerfile but kept, and its reason for existing (`tmap` v4 not on conda) is contradicted by `r-tmap>=4` now being in the yml | `env/installers/install_r_extra.sh:2`, `env/Dockerfile:29`, `env/gds_amd64.yml:144` | Dead, broken code | ✅ 2.4 |
| 10 | Almost all third-party tool downloads are unpinned (`latest` releases, `curl \| bash`, `@latest` npm/npx), so builds are non-reproducible and supply-chain-exposed | installers, `frontend_agent/Dockerfile`, `frontend_code/Dockerfile` | Reproducibility/security | ⛔ 4.1 |
...
**Status: DONE** — Per-layer cleanup; `fix-permissions` in-layer. ~3.2 GB smaller (18.9 → 15.7 GB, arm64). `9632985`, PR #105.
...
**Status: DONE** — pyppeteer deleted outright — jupyter-book v2 renders PDF via LaTeX, not a browser, so it was fully vestigial. `bf91656`, PR #104.
...
**Status: DONE** — Nine `-dev` header packages and a duplicate libpangocairo dropped. `97d060f`, PR #104.
...
**Status: DROPPED** — Superseded. Its only live change (gds_code latexmk apt hygiene) was mooted by `89ee39b`, which moved latexmk into base gds and deleted the gds_code block entirely. The code-server pin falls under 4.1 (won't-fix).
...
**Status: DONE** — Four `gds_py_explicit_*.txt` deleted. PR #118.
...
**Status: DONE** — `.gitpod.yml` deleted (maintainer call, not bumped); `frontend_code/compose.yml` `11.0alpha` → `11.0`. PR #118. One loose end below.
...
**Status: DONE** — `ENV LANG=C.UTF-8 LC_ALL=C.UTF-8` — two variables. `11fe264`.
...
**Status: DONE** — `install_r_extra.sh` deleted along with its commented-out Dockerfile invocation. PR #118.
...
**Status: DONE** — `url: "http://localhost:4000"`, with a note on overriding locally. PR #118.
...
**Status: DONE** — Dead `export` dropped, `JEKYLL_ENV=docker` inlined, cleanup moved to a new `website_clean` target. PR #118.
...
**Status: DONE** — All three rewritten: what the directory holds, which `make` target uses it, where the package list actually lives. PR #118.
...
**Status: DONE** — HTML comment headers naming the rendered URL added to both guides. PR #118.
...
**Status: PARTIAL — code landed #125, awaiting the two-arch build.** All 12 scripts in `env/installers/` now set `set -euo pipefail` (was 2 of 12). The `_lib.sh` helper is **deliberately still absent**: it is a second variable in a build that is expected to break, so it is a follow-up once the build is green — a failing `RUN` should have one suspect, not two.
...
Two known hazards were left to fail loudly rather than papered over, per the fix below: `vim +PlugInstall +qall` with no terminal, and `jupyter labextension disable`, which returns non-zero on some JupyterLab versions. If the build breaks, look there first. Deliberately deferred: the audit's own note says strict mode will surface latent failures needing a full two-arch build to diagnose, and no session so far has had Docker. Earlier partial progress from 4.2's lint triage: `install_tippecanoe.sh` gained a shebang, and the `export VAR=$(…)` return-value masking in `install_{quarto,typst,gpq}.sh` is fixed — which is precisely what lets `set -e` catch those failures once it lands. There is now also a **regression guard**: shellcheck runs on every PR at warning severity, so a new `et -e` cannot reach master unnoticed.
...
**Status: DONE** — The four `env/r/*.sh` deleted; glob narrowed to `COPY ./installers/*.sh`. PR #118.
...
**Status: DONE — amd64 UNVERIFIED** — Unified on Playwright for both arches with `chromium.executablePath()`; fallback cascade deleted. `97d060f`, plus an amd64 discovery fix in `0674a74`. **On the ✅\* marker:** the two-arch `make test` gate was never reported green, and `DECKTAPE_AMD64_HANDOFF.md` is still an untracked file in the working tree. Treat amd64 as unverified. **A concrete suspect surfaced during 3.4's `pipefail` audit (2026-09-08):** `decktape_wrapper.sh`'s `is_usable_browser` runs `head -n 20 "$candidate_path" | grep -q 'snap install chromium'` under `set -o pipefail`. `grep -q` exits on the first match, so `head` can take SIGPIPE and the pipeline reports 141 — meaning the `if` goes false and the function returns **usable for exactly the snap wrapper it exists to reject**, non-deterministically, depending on where the match falls. It would only bite where snap chromium exists, which is the amd64 path. Not fixed in #125: it is outside 3.4's scope and DeckTape's convolution is load-bearing (see the standing constraint). Worth checking first when the amd64 build finally runs. Note this was resolved *without* pinning, contrary to the proposed fix below — see the maintainer's no-pinning policy (4.1).
...
**Status: WON'T FIX** — Maintainer policy: this project tracks latest for everything, deliberately. Applies to code-server, Quarto, Typst, tippecanoe, DeckTape and the agent harnesses alike. 3.8 was solved without pinning for the same reason. Not to be re-proposed.
...
**Status: DONE** — All four: `--rm`, quoted `"$(CURDIR)"` mount, `aarch64`→`arm64` mapping, `.PHONY`, plus explicit `--build-arg BUILDARCH`. PR #118. The aarch64 gap was a real bug — arm hosts were tagging `gds:<date>_aarch64` and writing `stack_py_aarch64.*`.
...
**Status: DONE** — Binder-shim header added. PR #118.
...
**Status: DONE** — `.gitignore` virtualbox relics removed, themes aligned on `system`, compose placeholders commented, and the four `org.opencontainers.image.*` labels added (licence `BSD-3-Clause`, per LICENSE). PR #118.
...
- **3.6** — keep or delete `test_py_stack.ipynb` and `test_courses.ipynb` (per notebook).
- **4.4b** — the `--with-git-creds` credential-mount posture.
- **1.4b** — removing `rust`/`cython` from the ymls, once the evidence is in.
- **1.8** — migrating Pages off the committed `docs/` tree needs a one-time repo settings change.
- **Root `Dockerfile`** — the `gc` suffix in `darribas/gds:11.0gc` is undocumented in the repo and in that file's entire git history. Flagged in-file; confirm before the next release bump.

### Closed without action

- **4.1** (pinning) — won't-fix; the project tracks latest by policy.
- **1.7** — superseded by the latexmk move in `89ee39b`.
...
<details>
<summary>Original plan, 2026-07-08 (superseded)</summary>
