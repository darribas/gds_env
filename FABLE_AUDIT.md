...
Both are runtime-path bugs that no amount of static review had caught — the arm64 one needed a build, which is the standing argument for the validation debt below. Note this was resolved *without* pinning, contrary to the proposed fix below — see the maintainer's no-pinning policy (4.1).
